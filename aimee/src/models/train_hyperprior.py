...
import shutil
import sys
from model_hyperprior import ScaleHyper
...
def train_one_epoch(
    ...
    for i, (data) in enumerate(train_loader):
        ...
        target_size = 256
        padding_left = (target_size - d.shape[-1]) // 2
        padding_right = target_size - d.shape[-1] - padding_left
        padding_top = (target_size - d.shape[-2]) // 2
        padding_bottom = target_size - d.shape[-2] - padding_top
        d_pad = F.pad(d, (padding_left, padding_right, padding_top, padding_bottom), mode='constant', value=0)
        ...
        out_criterion = criterion(out_net, d_pad)
...
def main(argv):
    ...
    test_dataloader = DataLoader(train_dataset, batch_size=args.batch_size, num_workers=args.num_workers, shuffle=True, pin_memory=(device == "cuda"))
    print(train_loader)
